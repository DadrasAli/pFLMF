...
class FedAvgServer:
    ...
    def train_one_round(self):
        ...
        # Calculate average train loss for this round
        avg_train_loss = sum(
            client_package["eval_results"]["after"]["train"].loss
            for client_package in client_packages.values()
        ) / len(client_packages)
    ...
    def test_client_models(self):
        """The function for testing FL method's output."""
        self.testing = True
        ...
        if len(clients) > 0:
            if self.val_clients == self.train_clients == self.test_clients:
                ...
                # Calculate losses using 'all_clients' key when clients are the same
                ...
                # Calculate losses using appropriate keys when clients are different
    ...
    def save_losses(self):
        ...
        # Initialize lists to store centralized losses and accuracies
        centralized_train_losses = []
        centralized_val_losses = []
        centralized_test_losses = []
        centralized_train_accs = []
        centralized_val_accs = []
        centralized_test_accs = []
        epochs = []

        # Extract losses and accuracies from test_results
        for epoch in sorted(self.test_results.keys()):
            results = self.test_results[epoch]
            if 'all_clients' in results:
                epochs.append(epoch)
                
                # Get metrics objects from 'after' state
                train_metrics = results['all_clients']['after']['train']
                val_metrics = results['all_clients']['after']['val']
                test_metrics = results['all_clients']['after']['test']
                
                # Extract values directly from Metrics objects
                centralized_train_losses.append(train_metrics.loss)
                centralized_val_losses.append(val_metrics.loss)
                centralized_test_losses.append(test_metrics.loss)
                
                centralized_train_accs.append(train_metrics.accuracy)
                centralized_val_accs.append(val_metrics.accuracy)
                centralized_test_accs.append(test_metrics.accuracy)

        # Create DataFrame for losses
        loss_data = {
            'epoch': epochs,
            'centralized_train_loss': centralized_train_losses,
            'centralized_val_loss': centralized_val_losses,
            'centralized_test_loss': centralized_test_losses
        }

        # Create DataFrame for accuracies
        accuracy_data = {
            'epoch': epochs,
            'centralized_train_accuracy': centralized_train_accs,
            'centralized_val_accuracy': centralized_val_accs,
            'centralized_test_accuracy': centralized_test_accs
        }
        ...
        df_accuracy = df_accuracy.sort_values('epoch')
        
        # Save to CSV files
        df_loss.to_csv(self.output_dir / "centralized_losses.csv", index=False)
        df_accuracy.to_csv(self.output_dir / "centralized_accuracies.csv", index=False)
