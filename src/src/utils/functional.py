...
'''
## Changed because of running jobs on the Berzelius server
...
def get_optimal_cuda_device(use_cuda: bool) -> torch.device:
    """Dynamically select CUDA device (with the most free memory) for running FL
    experiment, compatible with MIG devices.
    ...
    best_gpu_id = np.argmax(gpu_memory)
    best_gpu_name = torch.cuda.get_device_name(best_gpu_id)
    free_memory_gb = gpu_memory[best_gpu_id] / (1024 ** 3)
    print(f"Selected CUDA device {best_gpu_id}: {best_gpu_name}, Free Memory: {free_memory_gb:.2f} GB")
