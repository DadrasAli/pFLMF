...
class LowRank_DNN(DecoupledModel):
    ...
    def __init__(self, rank, dataset, pretrained):
        ...
        total_params = (
            dims["fc1"][0] * dims["fc1"][1] + dims["fc1"][0] +
            dims["fc2"][0] * dims["fc2"][1] + dims["fc2"][0] +
            dims["fc3"][0] * dims["fc3"][1] + dims["fc3"][0]
        )
    ...
    def forward(self, x):
        ...
        idx = 0
        def next_weights(out_dim, in_dim):
            nonlocal idx
            w = flat_weights[idx:idx + out_dim * in_dim].view(out_dim, in_dim)
            idx += out_dim * in_dim
            b = flat_weights[idx:idx + out_dim]
            idx += out_dim
            return w, b

        fc1_w, fc1_b = next_weights(*self.param_shapes["fc1"])
        fc2_w, fc2_b = next_weights(*self.param_shapes["fc2"])
        fc3_w, fc3_b = next_weights(*self.param_shapes["fc3"])
...
class LowRank_CNN(DecoupledModel):
    feature_length = {
        "mnist": 784,
        "medmnistS": 784,
        "medmnistC": 784,
